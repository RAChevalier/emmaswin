		...
		if (!$all_results) {
			...
			while ($row = mysqli_fetch_assoc($all_results)) {
				...
				if ($table_answer == null || $table_answer == "") {
					$table_answer = "<form method='post' action='user_query/query_action.php?userid=" . @$userid . "' id='historyanswerform" . @$queryid . "' onsubmit=\"return confirm('Are you sure you want to submit this answer?');\"><textarea class='form-control answerbox' rows='2' name='answercontent' form='historyanswerform" . @$queryid . "' required='required' ></textarea><input type='hidden' name='answerquery' value='" . @$queryid . "' /><input type='submit' class='btn btn-default' value='Submit' /></form>";
				}
				
				// add new row to table for each additional In Progress query
				$all_queries_string .= "<tr><td><a href='main.html?userid=" . @$userid . "&queryid=" . @$queryid . "' title= 'Click to view this query'>" . $row["time"] . "</a></td><td><a href='https://www.google.com.au/search?q=" . $row["content"] . "' title='Click to view results in Google' target='_blank'>". $row["content"] . "</a></td><td>" . $table_location . "</td><td>"	. $table_audio . "</td><td>" . $table_image . "</td><td>" . $table_answer . "</td></tr>";
			}
