<?php
	...
	if (isset($_GET["userid"])) {
		// check whether the agent currently logged in is the one being assigned to
		$queryassignment =  "SELECT assignedto FROM $user_table where userid = '".$_GET['userid']."' and status = 1";
		$checkassignment = mysqli_query($conn,$queryassignment);
		$checkresult = mysqli_fetch_row($checkassignment);
		$agentassigned = $checkresult[0];

		if($agentassigned != $_SESSION['userid']){
			echo "<p>User is currently assigned to another agent. Please select another user.</p>"
		} else {
