<?php
	...
	if ($answer != null) {
		...
		$answer_area = "<form method='post' action='user_query/query_action.php?userid=" . @$_GET['userid'] . "' id='answerform' onsubmit=\"return confirm('Are you sure you want to submit this answer?');\">";
	...
	$content_string .= "</td></tr><tr><td><br /><form method='post' action='user_query/query_action.php' onsubmit=\"return confirm('Are you sure you want to mark all In Progress queries as Open?');\">";
	$content_string .= "<input type='hidden' name='allopen' id='allopen' value='" . @$_GET['userid'] . "' />";
	$content_string .= "<input type='submit' class='btn btn-default' value='Mark all In Progress queries as Open' />";
	
	$content_string .= "&nbsp;&nbsp;&nbsp;&nbsp;<button type='button' class='btn btn-default' onclick='displayResult(); displayMap()'>Load Google</button></form>";
